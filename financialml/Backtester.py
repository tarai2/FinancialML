...
class Backtester:
    ...
    def setPrice(self):
        ...
        limitAsk = act.query("side=='buy'").price.resample(
            "1s").max().to_frame(name="limitAsk")
        limitBid = act.query("side=='sell'").price.resample(
            "1s").min().to_frame(name="limitBid")
        # MO約定用Price: bookのPriceからSecondを1秒ごとにresample(last)
        bestAsk = book.loc[:, "askPrice1"].resample("1s").last().to_frame(
            name="bestAsk").fillna(method="ffill").dropna()
        bestBid = book.loc[:, "bidPrice1"].resample("1s").last().to_frame(
            name="bestBid").fillna(method="ffill").dropna()
        # 累積板厚 :投資行動からみて直近の秒足closeの値
        bidVol = book.loc[:, book.columns.str.contains("bidVol")].resample("1s").last(
        ).shift().cumsum(axis=1).fillna(method="ffill").fillna(method="ffill", axis=1)
        askVol = book.loc[:, book.columns.str.contains("askVol")].resample("1s").last(
        ).shift().cumsum(axis=1).fillna(method="ffill").fillna(method="ffill", axis=1)
        # 各レベルの価格
        bidPrices = book.loc[:, book.columns.str.contains("bidPrice")].resample(
        ...
        # attr作成
        self._bestAsk = idx.join(bestAsk)
        self._limitAsk = idx.join(limitAsk)
        self._bestBid = idx.join(bestBid)
        self._limitBid = idx.join(limitBid)
        self._midPrice = pd.DataFrame(
            (self._bestBid.values+self._bestAsk.values)/2, index=idx.index, columns=["midPrice"])
        self._bidVol = idx.join(bidVol)
        self._bidPrices = idx.join(bidPrices)
        self._askVol = idx.join(askVol)
        self._askPrices = idx.join(askPrices)
    ...
    def addFeature(self, data: pd.DataFrame, fillna=0):
        ...
        """
        assert "M8" in data.index.dtype.str,\
               "input data must have DatetimeIndex"
    ...
    def backtest(self, action: Callable, q_limit=0.01) -> List[np.array]:
        ...
        Q = np.zeros(self.T)
        trs_bid = np.zeros([self.T, 2])
        trs_buy = np.zeros([self.T, 2])
        trs_ask = np.zeros([self.T, 2])
        trs_sell = np.zeros([self.T, 2])
        pnl_close = np.zeros(self.T)
        pnl_open = np.zeros(self.T)
        api_count = np.zeros(self.T)

        # 情報@t
        feature = np.array(
            list(self._feature.reset_index().to_dict("index").values()))
        ...
        for t in np.arange(self.T):
            ...
            action(t, state, feature[t], bidPrices[t],
                   bidVol[t], askPrices[t], askVol[t])

            # check約定 : LO
            if state["bid"] and not np.isnan(limitBid[t]):
                trs_bid[t], state["bid"] = Backtester._checkLO(
                    t, state["bid"], limitBid[t], +1)
            if state["ask"] and not np.isnan(limitAsk[t]):
                trs_ask[t], state["ask"] = Backtester._checkLO(
                    t, state["ask"], limitAsk[t], -1)
