...
def get_tickinbbar(market_activity, initial_T=10):
    ...
    dP["flag"] = (1*(dP>0) - 1*(dP<0) + (dP==0).replace(True, np.nan) \
    ...
    for t,b,prob in zip(np.arange(dP.shape[0]), dP.flag.values, prob_positive):
        ...
        if np.abs(theta) > expected_T * np.abs(2*prob-1):
            theta=0; expected_T=expected_T*0.5 + (t-bar[-1])*0.5
            bar.append(t)
    return dP.index[bar]
...
def get_volbar(market_activity, threshold, isBase=True):
    ...
    """
    if isBase: cumAmount = market_activity.amount.cumsum()
    else: cumAmount = (market_activity.price*market_activity.amount).cumsum()
...
def cusum_filter(raw_series, theta, mean_func="exp", window=0.5):
    ...
    """
    assert "M8" in raw_series.index.dtype.str, "raw_series must have DatetimeIndex"
...
def fisher_exact_test(series, eff_pval=1, window=60):
    ...
    """
    df_effPval = pd.read_csv(os.path.dirname(__file__)+"/fisher_effPval.csv") #[W,theta_p,q]
    assert eff_pval in df_effPval.q.unique() and window in df_effPval.W.unique(),\
        "invalid value of eff_pval or window. these values must be included in fisher_effPval.csv"
    time_change, time_signal, direction =\
        _fisher_exact_test_bulk(series.values, df_effPval.query("W==@window and q==@eff_pval").theta_p.iloc[0], window)
    data = pd.DataFrame({
...
logsumVec = np.array([0] + [np.log(np.arange(1,k+1)).sum() for k in np.arange(1,1000)]) #log(k!)
@numba.jit(nopython=True)
        ...
        for div_lookback in range(1, W/2+1):
            ...
            if a*d - b*c > 0:
                ...
                for k in range(a, m+1):
                    p_value += np.exp(logsumVec[a+b]+logsumVec[c+d]+logsumVec[a+c]+logsumVec[b+d]-logsumVec[k]\
                                        -logsumVec[a+b-k]-logsumVec[a+c-k]-logsumVec[d-a+k]-logsumVec[n])
            else:
...
def get_first_signal(signal_time, lookforward):
    flags = _get_first_signal(signal_time.values.astype(float)/1e9, lookforward)
